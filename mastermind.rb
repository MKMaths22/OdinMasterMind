...
class Human
  ...
  def make_code
    codemaker ? make_code_or_guess : []
  end

  def make_guess
    codebreaker ? make_code_or_guess : []
  end
  ...
  def make_code_or_guess
    puts "Let's hope the computer can't crack this code." if codemaker
    puts 'Guess the code.' if codebreaker
...
class Computer
  ...
  def make_guess
    ...
    codebreaker ? remaining_possible_codes.sample : []
...
class FeedbackDisplayer
  ...
  def array_to_string(guess_array, feedback_array)
    ...
    feedback_string = if stringy_feedback == ''
                        "  feedback: No matches. \n"
                      else
                        "  feedback: #{feedback_array.compact.join(', ')}. \n"
                      end
...
until game_controller.turn_number == TURNS
